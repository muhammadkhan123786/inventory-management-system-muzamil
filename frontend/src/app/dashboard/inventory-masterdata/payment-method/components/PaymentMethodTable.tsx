...
import { Star, CreditCard, Trash2 } from "lucide-react";
import { IPaymentMethod } from "../../../../../../../common/suppliers/IPayment.method.interface";
import { toast } from "react-hot-toast";

interface Props {
  data: (IPaymentMethod & { _id: string })[];
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="pt-4">
                <TableActionButton
                  ...
                  onDelete={() => {
                    if (item.isDefault) {
                      return toast.error("Default methods cannot be deleted.");
                    }
                    onDelete(item._id);
                  }}
                />
            ...
            <tr key={item._id} className="hover:bg-[#ECFEFF] transition-colors">
              ...
              <td className="px-6 py-4 text-center">
                <TableActionButton
                  ...
                  onDelete={() => {
                    if (item.isDefault) {
                      return toast.error("Default payment methods cannot be deleted.");
                    }
                    onDelete(item._id);
                  }}
                />
