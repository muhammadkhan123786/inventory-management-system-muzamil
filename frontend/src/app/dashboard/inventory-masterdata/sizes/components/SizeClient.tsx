...
type SizeWithId = ISize & { _id: string };

export default function SizeClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<SizeWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<SizeWithId>("/sizes", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-7 text-white shadow-lg flex justify-between items-center animate-slideInLeft">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl font-bold">Size Configuration</h1>
              <p className="text-blue-100 text-lg">Manage product sizing variants</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex justify-between items-center mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">Configure product sizing variants and options</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="mt-4 text-gray-400 font-medium">Loading sizes...</p>
            </div>
          ) : (
            <>
