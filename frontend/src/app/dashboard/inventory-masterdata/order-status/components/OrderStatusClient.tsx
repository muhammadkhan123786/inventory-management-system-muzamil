...
import { ClipboardList, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
type OrderStatusWithId = IOrderStatus & { _id: string };

export default function OrderStatusClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<OrderStatusWithId | null>(null);
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<OrderStatusWithId>(
    "/order-status", 
    "orderStatus", 
    "Order Status", 
    currentPage, 
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<OrderStatusWithId>("/order-status", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<ClipboardList size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Order Statuses</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage lifecycle stages for orders</p>
            </div>
          </div>
          <button
        ...
        <StatsCards 
          ...
          labels={{
            total: "Total Order Statuses",
            active: "Active Statuses",
            inactive: "Inactive Statuses"
          }}
          icons={{ total: <ClipboardList size={24} /> }}
        />
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! shadow-sm rounded-b-2xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">Configure order workflow stages and default settings</p>
            </div>

            <div className="flex gap-2 bg-linear-to-r from-blue-50 to-cyan-50 p-1 rounded-lg border border-blue-200 w-full md:w-auto">
              <button onClick={() => setDisplayView("card")} className={`flex-1 md:flex-none px-3 h-8 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${displayView === "card" ? "bg-linear-to-r from-blue-500 to-teal-600 text-white shadow-lg" : "text-gray-600 hover:text-blue-600 hover:bg-[#10b981]"}`}>
                <LayoutGrid size={16} /> <span className="text-sm">Grid</span>
              </button>
              <button onClick={() => setDisplayView("table")} className={`flex-1 md:flex-none px-3 h-8 rounded-lg font-bold flex items-center justify-center gap-2 transition-all ${displayView === "table" ? "bg-linear-to-r from-blue-500 to-teal-600 text-white shadow-lg" : "text-gray-600 hover:text-blue-600 hover:bg-[#10b981]"}`}>
