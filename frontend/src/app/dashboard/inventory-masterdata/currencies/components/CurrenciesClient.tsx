...
import { Coins, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
type CurrencyWithId = ICurrency & { _id: string };

export default function CurrenciesClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<CurrencyWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<CurrencyWithId>("/currencies", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Coins size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Currencies</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage global currencies and exchange rates</p>
            </div>
          </div>
          <button
        ...
        <StatsCards 
          ...
          labels={{
            ...
            inactive: "Inactive Currencies"
          }}
          icons={{ total: <Coins size={24} /> }}
        />
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! shadow-sm rounded-b-2xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">Configure active currencies for registration and financial tracking</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="mt-4 text-gray-400 font-medium">Loading currencies...</p>
            </div>
          ) : (
            <>
              ...
              <CurrenciesTable
                ...
                onEdit={(item) => { setEditingData(item as CurrencyWithId); setShowForm(true); }}
