...
import { Handshake, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
export default function PricingAgreementClient() {
  ...
  const { 
    ...
    deleteItem, 
    updateItem 
  } = useFormActions<PricingAgreementWithId>(
    "/pricing-agreement", 
    "pricingAgreements", 
    "Pricing Agreement", 
    currentPage, 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-6 md:p-7 text-white shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-slideInLeft">
          <div className="flex items-center gap-4 w-full md:w-auto">
            <AnimatedIcon icon={<Handshake size={32} className="text-white" />} />
            <div className="flex-1 md:flex-none">
              <h1 className="text-3xl md:text-4xl font-bold">Pricing Agreements</h1>
              <p className="text-blue-100 text-sm md:text-lg">Manage contract pricing terms with suppliers</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-4 mb-6">
            <div className="space-y-1">
              <h2 className="text-2xl font-bold bg-linear-to-r from-blue-600 to-teal-600 bg-clip-text text-transparent">Agreement Categories</h2>
              <p className="text-sm text-gray-500">Configure pricing terms for vendor management</p>
            </div>
