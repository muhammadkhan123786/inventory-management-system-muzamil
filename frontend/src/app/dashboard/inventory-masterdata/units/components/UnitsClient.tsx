...
import { Ruler, Plus, Search, Loader2, LayoutGrid, Table2 } from "lucide-react";
...
type UnitWithId = IUnit & { _id: string };

export default function UnitsClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingData, setEditingData] = useState<UnitWithId | null>(null);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const allDataRes = await getAll<UnitWithId>("/units", {
          limit: "1000", 
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-linear-to-r from-blue-600 via-cyan-500 to-teal-600 rounded-2xl p-7 text-white shadow-lg flex justify-between items-center animate-slideInLeft">
          <div className="flex items-center gap-4">
            <AnimatedIcon icon={<Ruler size={32} className="text-white" />} />
            <div>
              <h1 className="text-4xl font-bold">Unit Measurement</h1>
              <p className="text-blue-100 text-lg">Manage product units and scaling</p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white p-5 pt-9 border-t-4! border-[#2B7FFF]! ">
          <div className="flex justify-between items-center mb-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">Configure product measurement units and variants</p>
            </div>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center py-20">
              ...
              <p className="mt-4 text-gray-400 font-medium">Loading units...</p>
            </div>
          ) : (
            <>
