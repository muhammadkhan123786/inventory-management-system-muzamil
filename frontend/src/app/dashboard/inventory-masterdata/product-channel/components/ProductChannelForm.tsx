...
interface Props {
    editingData: (IChannel & { _id?: string }) | null;
    onClose: () => void;
    onRefresh: () => void; // Optional in usage
    themeColor: string;
}

const ProductChannelForm = ({ editingData, onClose, themeColor }: Props) => {
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 p-4">
                <FormInput
                    label="Channel Name *"
                    placeholder="e.g. Online Store, Retail, Wholesale"
                ...
                <FormButton
                    type="submit"
                    label={editingData ? "Update Channel" : "Create"}
                    icon={<Save size={20} />}
                    loading={isSaving}
                    themeColor={themeColor}
                    onCancel={onClose}
...
export default ProductChannelForm;