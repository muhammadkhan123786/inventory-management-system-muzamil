...
interface Props {
  ...
  onEdit: (item: IChannel & { _id: string }) => void;
...
const ProductChannelTable = ({ data, displayView, onEdit, onDelete, onStatusChange, themeColor }: Props) => {
  if (displayView === "card") {
    return (
    ...
    <div className="bg-white mt-8 shadow-lg border border-gray-200 overflow-x-auto rounded-lg">
      <table className="w-full text-[16px] text-left min-w-max">
        ...
        <tbody className="divide-y divide-gray-100">
          ...
          {data.length === 0 && (
            <tr>
              <td colSpan={4} className="text-center py-10 text-gray-400">No channels found.</td>
            </tr>
          )}
        </tbody>
      </table>
