...
// ─── KEY FIXES ────────────────────────────────────────────────────────────────
// 1. ALL axios calls use BASE_URL + Bearer token  ← price history nahi aata tha
// 2. Stats bar added at top
// 3. Gradient UI throughout
// ─────────────────────────────────────────────────────────────────────────────
"use client";
...
export function SupplierPricingTab({ supplierId, supplierName, userId }: Props) {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ BASE_URL + token — same as every other API call in the project
      ...
      // Fetch last price change for each — ✅ BASE_URL + token
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-gray-400 border-2 border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="text-sm font-medium">
            {rows.length === 0 ? "No products linked to this supplier" : "No results"}
          </p>
          {rows.length === 0 && (
        ...
        <div className="border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          {/* Header */}
          <div className="grid grid-cols-[2.5fr_1fr_1.2fr_1.2fr_0.8fr_0.8fr_auto] bg-gradient-to-r from-gray-50 to-slate-50 border-b px-4 py-2.5">
            {["Product", "SKU", "Cost Price", "Selling Price", "Margin", "Stock", "Actions"].map(h => (
              <div key={h} className="text-[11px] font-bold uppercase tracking-wider text-gray-500">{h}</div>
            ))}
          </div>
            ...
            {filtered.map(row => {
              ...
              return (
                <div
                  ...
                  className="grid grid-cols-[2.5fr_1fr_1.2fr_1.2fr_0.8fr_0.8fr_auto] px-4 py-3.5 items-center hover:bg-indigo-50/30 transition-colors"
                >
                  {/* Product */}
                  <div>
                    <p className="text-sm font-semibold text-gray-800 truncate">{row.productName}</p>
                    {row.leadTimeDays > 0 && (
                      <p className="text-xs text-gray-400 mt-0.5">Lead: {row.leadTimeDays}d</p>
                    )}
                  </div>

                  {/* SKU */}
                  <div>
                    <span className="text-xs font-mono text-gray-600 bg-gray-100 px-2 py-0.5 rounded-md">{row.sku}</span>
                  </div>
                  ...
                  <div>
                    ...
                    {row.lastChange?.changedAt && (
                      <p className="text-[11px] text-gray-400 mt-0.5">
                        {new Date(row.lastChange.changedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    <span className={`text-sm font-semibold ${
                      row.stockQty === 0 ? "text-red-600" :
                      row.stockQty < 10  ? "text-amber-600" : "text-gray-700"
                    }`}>
                      {row.stockQty}
                    </span>
                    {row.stockQty === 0 && <p className="text-[11px] text-red-500">Out of stock</p>}
                    {row.stockQty > 0 && row.stockQty < 10 && <p className="text-[11px] text-amber-500">Low stock</p>}
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-violet-600 bg-violet-50 hover:bg-violet-100 rounded-lg border border-violet-200 transition-colors"
                    >
                      <History className="h-3.5 w-3.5" /> History
                    </button>
                    <button
                      onClick={() => setUpdateModal({ open: true, row })}
                      className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-white bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 rounded-lg transition-all"
                    >
