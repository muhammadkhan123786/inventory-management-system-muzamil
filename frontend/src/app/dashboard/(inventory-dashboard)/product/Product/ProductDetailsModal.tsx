...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-100 last:border-0">
      ...
      <span
        className={`text-sm font-semibold ${color} ${mono ? "font-mono" : ""} break-all`}
      >
        {value}
      </span>
    </div>
...
function StockCard({ label, value, color, border, sub }: StockCardProps) {
  return (
    <div
      className={`bg-white rounded-xl p-4 border ${border} shadow-sm hover:shadow-md transition-shadow`}
    >
...
function PriceRow({
  ...
  return (
    <div className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
      ...
      <span
        className={`${bold ? "font-bold" : "font-semibold"} ${color} ${size === "lg" ? "text-xl" : "text-base"}`}
      >
        {value}
      </span>
    </div>
          ...
          {(
            ...
            .map((tab) => {
              ...
              const icons: Record<DescTab, React.ReactNode> = {
                description: (
                  <FileText
                    className={`h-3.5 w-3.5 ${activeTab === t ? "text-blue-600" : "text-gray-400"}`}
                  />
                ),
                specifications: (
                  <ListChecks
        ...
        <div className="pt-3 border-t border-gray-100">
          ...
          <div className="flex flex-wrap gap-2">
            {extractedFeatures.slice(0, 4).map((f, i) => (
              <span
                key={i}
                className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-full text-xs border border-blue-200 flex items-center gap-1"
              >
...
export default function ProductDetailsModal({
  ...
  const getCategoryName = (catId: any): string => {
    const id =
      typeof catId === "string" ? catId : catId?.id || catId?._id || "";
  ...
  const sellingPrice =
    pricing?.sellingPrice ?? (product as any).sellingPrice ?? 0;
  ...
  const profitPct =
    costPrice > 0 ? ((profitMargin / costPrice) * 100).toFixed(1) : "0.0";
            ...
            <div className="flex-1 min-w-0">
              ...
              <DialogDescription className="text-white/80 text-sm mt-1 flex flex-wrap gap-3">
                ...
                {(product as any).modelNumber && (
                  <span>Model: {(product as any).modelNumber}</span>
                )}
              </DialogDescription>
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                ...
                <Badge
                  className={`px-2.5 py-1 text-xs ${product.status === "active" ? "bg-emerald-400 text-emerald-900" : "bg-gray-400 text-gray-900"}`}
                >
                  {product.status}
                </Badge>
                <Badge
                  className={`${getStockBadge(stockStatus || product.stockStatus).class} px-2.5 py-1 text-xs`}
                >
                  {(stockStatus || product.stockStatus)?.replace("-", " ")}
                </Badge>
                {isLow && (
                  <Badge className="bg-red-500 text-white gap-1 px-2.5 py-1 text-xs">
                    <AlertTriangle className="h-3 w-3" /> Low Stock
                  </Badge>
                )}
                {vatExempt && (
                  <Badge className="bg-purple-400 text-purple-900 px-2.5 py-1 text-xs">
                    VAT Exempt
                  </Badge>
                )}
              </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(product as any).tags.map((t: string, i: number) => (
                          <span
                            key={i}
                            className="text-xs px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full border border-blue-100"
                          >
                            {t}
                          </span>
                        ))}
            ...
            <Card className="border-0 shadow-sm rounded-2xl">
              <CardContent className="p-5">
                ...
                <InfoRow
                  icon={BarcodeIcon}
                  label="Product SKU"
                  value={product.sku ?? (product as any).sku}
                  mono
                />
                <InfoRow
                  icon={Hash}
                ...
                <InfoRow
                  icon={Info}
                  label="Model No."
                  value={(product as any).modelNumber}
                  mono
                />
      ...
      Barcode
            ...
            <Card className="border-0 shadow-sm rounded-2xl">
              <CardContent className="p-5">
                ...
                <PriceRow
                  label="Retail Price"
                  value={fmt(retailPrice)}
                  color="text-indigo-600"
                />
                <PriceRow
                  label="Profit Margin"
                  value={`${fmt(profitMargin)} (${profitPct}%)`}
                  color={
                ...
                <div className="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-2">
                  ...
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full border ${vatExempt ? "bg-purple-50 text-purple-700 border-purple-200" : "bg-gray-50 text-gray-500 border-gray-200"}`}
                  >
            ...
            <CardContent className="p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                <StockCard
                  label="In Stock"
                  value={stockQty}
                  color={isLow ? "text-red-600" : "text-indigo-700"}
                  border={isLow ? "border-red-200" : "border-indigo-200"}
                  sub="current qty"
                />
                <StockCard
                  label="Min Level"
                ...
                {warranty.warrantyType || warranty.warrantyPeriod ? (
                  <>
                    ...
                    <InfoRow
                      ...
                      label="Period"
                      value={
                        WARRANTY_LABELS[warranty.warrantyPeriod] ??
            ...
            <div className="p-4 space-y-3">
              ...
              {((product as any).attributes ?? []).map(
                (variant: any, idx: number) => {
                  const attrEntries: [string, unknown][] = Object.entries(
                    variant.attributes ?? {},
                  );
                  ...
                  return (
                    <Card
                      key={variant._id ?? idx}
                      className="border border-gray-100 hover:border-indigo-200 transition-all"
                    >
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            ...
            <Button
              ...
              onClick={() => {
                if (window.confirm("Are you sure?")) onDelete(product);
              }}
            >
