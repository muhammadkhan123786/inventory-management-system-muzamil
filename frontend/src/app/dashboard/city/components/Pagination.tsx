        ...
        <div className="flex items-center justify-between px-4 py-4 bg-white border border-gray-100 mt-6 rounded-2xl shadow-sm">
            ...
            <div className="flex flex-1 justify-between sm:justify-end items-center gap-2">
                ...
                <div className="hidden md:flex gap-1">
                    {currentPage > 3 && (
                        <button onClick={() => onPageChange(1)} className="w-10 h-10 rounded-xl text-sm border hover:bg-gray-50">1</button>
                    )}
                    {currentPage > 4 && <span className="px-2 self-center text-gray-400">...</span>}
                    ...
                    {currentPage < totalPages - 3 && <span className="px-2 self-center text-gray-400">...</span>}
                    {currentPage < totalPages - 2 && (
                        <button onClick={() => onPageChange(totalPages)} className="w-10 h-10 rounded-xl text-sm border hover:bg-gray-50">{totalPages}</button>
                    )}
                </div>
